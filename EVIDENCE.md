...
# MangoUpsideDownCamera 1.1.0：证据与边界
...
## 1.0.0 真机结果：系统相机没有跟着转；1.1.0 的诊断，不是修复

用户装机后做了两组测试：(1) 先把手机翻过来，再打开系统相机（冷启动，`supportedInterfaceOrientations` 在设备已经是倒置状态时才第一次被查询）；(2) 先正常打开相机（此时是竖屏），相机开着的时候才把手机翻过去（实时旋转，考验系统会不会在界面已经显示之后重新查询一次这个方法）。两种测试都失败——取景预览没有跟着转。

**两种测试都失败这件事本身有排除力**：如果原因是 iOS 16 那类"界面已经显示后，系统不会主动为倒置这个方向重新发起查询"的行为（真实存在，查证过开发者论坛上明确的类似报告——不是凭空猜的），那冷启动这一种理应不受影响，因为冷启动时这个方法本来就是在设备已经倒置的状态下才第一次被调用，不存在"重新查询"这个环节。冷启动也失败，说明问题出在更早的一步：**很可能相机 App 自己重写了 `supportedInterfaceOrientations`，这次 hook 的基类实现根本没被调用到**——这正是 1.0.0 版本自己在注释里已经提前说清楚、且用户当时就知情接受的那个结构性限制（见上面"机制上的真实边界"一节）。

**没有直接升级到"扫描全部运行时、把所有重写过这个方法的类都找出来逐个挂 hook"（原计划里的"全量覆盖"方案）**。理由：这个方案的改动范围和风险都远大于目前已有的任何一步——不仅要在每个进程里扫描全部已加载的类（性能成本不小），还要对扫描出来的每一个类都单独打补丁，而目前完全不知道具体是哪个类、有多少个类符合条件。在没有真实数据支撑的情况下就扩大到"全部"这个范围，正是灵动岛 alpha1-8 那几个版本吃过的同一种亏——先推理、再直接下手 hook，而不是先看运行时真实状态。所以这次先加一个纯只读、不修改任何行为的诊断，把相机进程里到底是哪个类重写了这个方法**精确地列出来**，之后才有针对性地只对那一个（或那几个）真实存在的类下手，而不是不分青红皂白地把整个运行时全部改一遍。

**诊断本身的设计**：

- **新增 `TracePath` 开关**（`/var/mobile/Library/Preferences/MangoUpsideDownCamera.trace`），跟 `MangoUpsideDownWorld` 的 `.trace` 同一个建法，但用途不同：这个包目前没有常驻的定时器/轮询循环（每个 App 进程只在真的被系统问到方向时才会跑到 hook 代码），所以这不是一个持续轮询的开关，只在 `Install()`（也就是这个 App 进程刚启动、dylib 刚加载的那一刻）读取一次，决定这次要不要跑诊断——因此已经在跑的 App 进程不会因为之后创建了这个文件而补跑一次，需要先完全退出目标 App 再重新打开才能让这次诊断在它身上生效。
- **新增 `LogOverridingClasses()`**：只读、只在 trace 开启时于 `Install()` 里跑一次。用 `objc_copyClassList` 遍历这个进程里所有已加载的类，只保留 `UIViewController` 的子类（排除 `UIViewController` 自己，因为那个已经被现有 hook 覆盖了），且用 `class_copyMethodList` 确认这个类**直接定义**（不是继承）了自己的 `supportedInterfaceOrientations`——跟 `MangoUpsideDownWorld` 项目里 `LogOwnMethods()`/`CLASSDUMP` 区分"定义"和"继承"用的是完全一样的判据（各自独立实现，不共享代码）。命中的类名去重列出，记一条 `CLASSDUMP overriders=<数量> classes=<类名列表>`，最多列 40 个防止刷屏。这个判据只看 `UIViewController` 子类：如果真正的重写者是一个包裹相机界面的容器控制器（比如某个私有的导航控制器子类，用于把多个子界面各自的方向要求汇总成一个结果——这是 `UINavigationController` 一类容器类的标准做法，苹果自己的文档里这类容器也会重写这个方法），同样会被这次扫描抓到，因为这类容器本身也是 `UIViewController` 的子类。
- **新增持久化日志**：这个文件之前只有裸的 `NSLog()`，需要 Mac 上的 Console.app 或者设备自带的系统日志工具才能看，对用户不方便。新增 `Log()` 函数，结构上模仿 `MangoUpsideDownWorld` 的同名函数（同样的 `mkdir`/`O_APPEND`/512KB 轮转封顶，各自独立实现，不共享代码），写到 `/var/mobile/Library/Logs/MangoUpsideDownCamera.log`。因为这个 dylib 会被注入进很多个不同的 App 进程，大家共用同一个日志文件路径，所以每一行都在前面标注了写入这一行的 App 的 bundle ID（`Install()` 时缓存一次，不用每次调用都重新查询）——不然日志会是所有 App 的记录混在一起、分不清谁是谁。
- **新增 hook 内部的调用记录**：`HookSupportedOrientations` 现在在 trace 打开时会记两类行——`CALL class=... orig=...`（每次被调用都记，不管有没有真的改动返回值，用来确认这个 hook 本身有没有被这个类调用到）和 `HIT class=... device=... patched=...`（真的往返回值里加了倒置支持位的那一刻）。这两类日志能直接看出：如果测试时倒置状态下打开过相机，`CALL` 行里有没有出现过相机自己的界面类——如果完全没有，说明这次 hook 在基类层面压根没被相机的界面类调用到，进一步坐实"相机自己重写了这个方法"这个结论。

不影响任何已有行为——`HookSupportedOrientations` 原有的判断逻辑（`InvertOrientations`/`Disabled`/`UIDevice.orientation`）一字未改，这次全部是新增的诊断代码，且全部由 `Tracing` 这一个开关统一控制，默认关闭。

**性能提示已经写进 README**：这个诊断是运行时全量扫描，不是常驻轻量诊断，如果长期开着 trace，会让手机上所有装了这个插件的 App 启动都变慢一点——不是可以一直放着的东西，测完应该删除 `.trace` 文件。

## 尚未处理（这个新包自己的，跟上面 `MangoUpsideDownWorld` 的清单分开记）

- 相机进程里真正重写 `supportedInterfaceOrientations` 的具体类名仍然未知，等待用户提供 1.1.0 的 `CLASSDUMP`/`CALL`/`HIT` 真机日志。有了具体类名之后，才能设计下一版针对性的 hook。
