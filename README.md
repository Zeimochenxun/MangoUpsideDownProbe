# MangoUpsideDownWorld 0.15.1-alpha15b
...
- **alpha15 是三件事一起做的，用户已知情并要求接受相应风险**：
  1. **全局 `UITouch` 定位修复尝试**——alpha11 发现灵动岛的手势类只自定义了 `touchesMoved:withEvent:`，而 alpha11 加的 `LPROBE`（挂在 `UIGestureRecognizer.locationInView:`/`locationOfTouch:inView:` 上）在真机连续拖动中几乎不触发，说明真正读位置的大概率不是这两个方法。下一个候选是 `UITouch` 自己（另一个类，同名方法）的 `locationInView:`/`previousLocationInView:`。这次不再分阶段（先探测再改），直接把读值和改值合在一次改动里做。**这是这份文件历史上第一个不局限于某个已验证私有类的 hook**——`UITouch` 是公开类，这个方法是 SpringBoard 进程内所有读触摸位置的必经之路，不只是灵动岛，还包括主屏、控制中心、通知中心、锁屏密码输入、Face ID 提示、多任务、Spotlight 键盘等等。靠两层限定把影响面收紧到灵动岛：新增的顶层开关 `ManualOn`（见下）必须打开，且 `InsideTurnedRoot(view)`（复用已有函数，未改）确认这个 view 确实在被 World 转过的子树里——密码输入、Face ID、主屏等永远不在这棵子树里，跟 `TurnDelta` 已经依赖的是同一条判断，不是新推理。**这次改动不保证修好方向**：如果灵动岛读位置走的是别的路径（原始 ivar 或另一个私有方法），这次改动不会有任何效果，但风险已经被上面两层限定收紧了。
  2. **`ManualOn`：全局手动开关**，默认关闭，且**每次 respring 都不保留状态**，重新回到关闭。开关本身是屏幕左下角一个常驻小圆点（不受 `Tracing` 影响，始终存在），点一下切换开/关，颜色从深灰"OFF"变成红色"ON"。这不是只管新加的两项风险，而是**整个 World 现在都需要先点开关才会工作**——原来只要方向倒置就自动生效的位置/内容修正、命中测试兜底、手势方向修正，现在都额外多了这一条门槛。倒置状态下屏幕物理翻转会让这个按钮和悬浮追踪球一样跟着转正，保证倒过来看时仍读得出"ON"/"OFF"。
  3. **音量键变成不依赖触控的物理恢复通道**：只有开关开着（`ManualOn`）时音量键才会被接管，关着时音量键行为完全不变。开着的时候：**短按完全被吞掉，不调整音量、不做任何事**；**长按其中一个键会恢复正常并自动关闭开关**（等同于建 `.disabled` 文件的效果，但不需要respring、立即生效）；**长按另一个键会直接 respring**（`exit(0)`，SpringBoard 由 launchd 的 KeepAlive 自动重新拉起，这是社区通用的 respring 按钮实现方式，不是新的私有 API）。**哪个键对应哪个动作，这次没有硬编码猜测**：`SBVolumeControl` 用一个未公开含义的数字区分音量上/下键，硬编码猜错的后果是长按你以为是"恢复"的键却触发了重启（反过来也一样）。改成开关打开后**第一次**长按任意一个键，就把那个键记成"上键"（同时立即执行恢复动作），本次开机内固定；如果第一次长按的其实是物理下键，后果只是这一次给了恢复而不是重启，按一下另一个键就重启，不会更糟——恢复本身就比重启更保守，这个误判自己会在下一次按键时纠正。详见 EVIDENCE.md。
- **alpha15b 修了 alpha15 真机反馈的开关按钮点不动**：原因是这个按钮的窗口在 `Install()` 里直接同步创建，而 `Install()` 本身跑在 SpringBoard 刚启动的阶段，这时候系统的窗口场景可能还没连接好，代码退到了一个不依赖场景的兼容创建方式——这种方式能显示出来，但在现代 iOS 上往往收不到触摸事件。悬浮追踪球用几乎一样的代码却一直正常，区别是它只在你手动开 trace 时才创建，那时系统早就启动完了。修法：按钮创建改成不再有这条兼容退路，只有真的连接到窗口场景才创建，创建时机也从 `Install()` 移到 `Reconcile()` 里——这样它会跟着已有的 250ms 定时器不断重试，直到场景真的就位。同时加了两行诊断日志：`ARM READY`（按钮真正创建成功那一刻）和 `ARM TAP now=ON/OFF`（每次点按钮，不管有没有其它问题都会先记下点没点到）——如果这次还有类似问题，直接看有没有这两行就能立刻区分"根本没点到"还是"点到了但后面逻辑另有问题"。
...
## alpha15：手动开关与音量键恢复通道（先做这一步，再测倒置）

从这一版起，**倒置不再自动生效**——World 的全部功能（位置/内容修正、命中测试兜底、手势方向修正，以及本版新加的全局触摸定位修复）现在都需要先手动打开一个开关才会工作。装好、respring 之后，屏幕左下角会出现一个常驻的小圆点（不需要开 `.trace`，一直都在），显示"OFF"：

1. **先不要点开关**，正常竖屏/横屏操作一遍，确认这一步（开关还是 OFF 的状态）跟以前版本行为一致——包括音量键完全正常。
2. **在还没进入倒置、双手都能自由操作的安全状态下**，点一下这个开关，看它变成红色"ON"。
3. **立刻**（不要先做别的操作）单独长按*一个*音量键 0.6 秒以上。这是本版唯一需要你手动做的一步校准：`SBVolumeControl` 用来区分音量上/下键的数字在 iOS 16.5 上没有公开文档，所以第一次长按哪个键，这次开机期间那个键就被记成"恢复正常"键，另一个键自动成为"重启 SpringBoard"键。取日志确认 `VOLUME CALIBRATED up-type=...` 这一行，记下你长按的是物理上键还是下键。
4. 松手后开关应该已经自动变回"OFF"，World 的效果（如果之前处于倒置状态）也应该已经复原——这一次长按同时完成了"校准"和"恢复"两件事。
5. 再点一次开关变 ON，长按**另一个**音量键，确认 SpringBoard 会 respring（黑屏几秒后重新出现主屏）。
6. Respring 之后开关会重新回到 OFF（不保留状态，这是设计如此），音量键立刻恢复正常——**在开关是 OFF 的状态下，任何时候按音量键都应该跟没装这个补丁一样**，如果不是，请立刻说。
7. 只有确认过上面 3-6 步都符合预期后，才进入倒置测试灵动岛拖动方向；测试期间保持开关是 ON 的（否则 World 不生效）。
8. **如果长按音量键校准/恢复/重启这套机制本身没有按预期工作**（比如长按了但什么都没发生，或者两次长按都触发了同一个动作），**不要继续测试倒置**——先按 README 开头"先准备恢复途径，再安装"里的 SSH/Filza/强制重启流程处理，这条物理通道本身失灵意味着后面每一步都失去了预留的退路。
9. 灵动岛拖动方向的测试方法不变（见上面"安装与验收"里的 `GESTURE` 行说明），但本版额外多了一类日志：取 `TOUCHFIX api=UITouch.locationInView:/previousLocationInView: raw=... turned=...`，只在开关 ON 且拖动的视图确实在被 World 转过的子树里时才会出现。**如果方向修好了，看这次日志里有没有 `TOUCHFIX` 行一起发回来**；**如果方向依旧是反的，也请说明有没有 `TOUCHFIX` 行**——没有的话说明这次 hook 的方法同样没被调用，跟 alpha7/alpha11 排除 `translationInView:`/`velocityInView:`/`UIGestureRecognizer.locationInView:` 是同一类结果，不是回归。
