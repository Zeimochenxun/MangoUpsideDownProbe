# MangoUpsideDownWorld 0.15.2-alpha15c
...
- **alpha15b 真机结果**：`ARM READY` 出现了（场景创建这一步确认修好），但确认点了按钮之后**没有** `ARM TAP`。这说明问题不在场景创建时机，而在"点击这个动作本身"——具体是命中测试没让触摸传到这个视图，还是触摸传到了但手势识别器本身没触发，这两种情况区分不出来。**alpha15c** 加了一层更底层的诊断：给按钮换了个自定义视图类，直接重写 `touchesBegan:withEvent:`（完全绕开 `UIGestureRecognizer` 自己的状态机），点按钮时会先记一行 `ARM TOUCHBEGAN`，跟原来的手势识别器完全独立、互不影响。下次装上之后点按钮，看日志：**只有 `ARM TOUCHBEGAN` 没有 `ARM TAP`**——说明触摸确实传到了这个视图，问题在手势识别器本身；**两行都没有**——说明触摸压根没传到这个视图，要往窗口层级/命中测试那边查。
  - 同时有两个现在就能免费做、不需要重新装包的判断实验，麻烦顺手也帮忙确认一下：**开着 trace 时，点一下右下角原来那个悬浮追踪球，看它会不会展开成一块深色面板**——如果这个也点不动，说明是这台设备上所有这类悬浮窗按钮的通病，不是这次新加按钮单独的问题；**把 trace 关掉（删掉 `.trace` 文件）之后再点左下角的开关**——如果这时候突然能点动了，说明是两个悬浮窗叠在一起时互相挡住了，不是按钮本身的问题。
