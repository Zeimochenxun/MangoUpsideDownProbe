...
static void WPRestore(UIView *v,MWPState *s,NSString *reason){
    if(!s.applied)return;
    // Restore only a value that is still exactly ours. If UIKit/Mango already
    // relaid the container, accept its new center instead of overwriting it
    // with stale geometry.
...
static void WPUpdate(UIView *v){
    ...
    // If the system changed center since our write, stop treating the old
    // placement as owned. The current center becomes the new baseline.
    ...
    // Never move anything until World's window half-turn is actually active.
