...
with tarfile.open(fileobj=io.BytesIO(subprocess.check_output(['dpkg-deb', '--fsys-tarfile', package]))) as t:
    ...
    for required in [b'MSHookMessageEx', b'SBSystemApertureContainerView', b'Status.log', b'content-not-empty-hidden']:
        assert required in data, required
    for forbidden in [b'MSHookFunction', b'initWithFrame:groupName:filterType:', b'locationInView:', b'translationInView:']:
