...
空闲时，在原有灵动岛容器里添加补充背景。运行时类与构造签名经验证，并且 Mango 液态玻璃设置显示为开启，或在灵动岛活动中亲眼观察到 Mango 原始玻璃视图时，补充背景使用 Mango 自己的 `MGLiveBackdropView`，`groupName=Island`、`filterType=go.mangoos.island`。Mango 的配置优先从 RootHide 映射路径读取，再回退系统路径。若开机后还没有观察到活动玻璃，且设置读取不到，则暂时使用系统磨砂；首次看到原有 Mango 玻璃后自动升级材质，无需 Respring。原版 0.2.0 即使后来检测到玻璃也不会升级，这是本次修复的核心。本插件只读取设置，不修改原设置或授权流程。
...
状态日志：`/var/mobile/Library/Logs/MangoIdleIsland/Status.log`，只在状态变化时写入，约 64 KiB 循环截断，不记录通知文字。`[GLASS] constructor-verified=1` 表示类来源与签名通过；`[GLASS] original-island-glass-observed` 表示在活动岛内发现原有玻璃；`[BACKGROUND] kind=Mango-glass source=observed upgraded=1` 表示已由系统磨砂升级为真正的 Mango 玻璃。若仍为 `kind=UIKit-fallback`，可从日志区分是否从未见过活动玻璃或构造方法不可用。`state=background` 为无可见活动内容；`state=activity` 是过渡/活动期间的连续交接。每次 Respring 有新的 SESSION。
...
不注入 backboardd；不修改 Mango 文件、授权逻辑、偏好或系统方向。不使用固定函数地址。仅支持 iOS 16.5.0，其余系统自动退出。无法识别关键类或 hook 方法签名时不安装 Hook。安装 0.3.0 将升级本包 0.2.0；不需叠装。
